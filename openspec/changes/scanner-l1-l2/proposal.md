...
- Add Layer 2 feature extraction computing all eight architecture-doc features: `rsi_14`, `volume_ratio` (pace-adjusted intraday), `atr_pct`, `price_vs_50ma`, `compression_score`, `short_interest`, `sector_momentum`, `regime_tag`.
...
- Persist qualifying scan results as `tradingstack.ScanResult` rows (the model landed by `trading-stack-schema`) via GORM — Layer 1 rejects never reach persistence.
