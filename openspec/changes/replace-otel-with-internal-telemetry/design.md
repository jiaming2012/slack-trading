...
**D4b — Ack channels.** Acking is one REST call — `POST /telemetry/alerts/{id}/ack` — with two front doors: an `ack <id>` command handled by the Slack inbound handler (`src/go/api/slack`), and a `task alert:ack ID=<id>` target that curls the endpoint. Both record `acked_via` (slack|cli). Unacked alerts are flagged in `task telemetry:status`. *Alternative rejected:* Slack interactive buttons — requires Slack app interactivity callbacks and signing-secret verification we don't have.

*Mid-run discovery (artifacts amended, not silently deviated):* the "existing" Slack slash-command switch turned out to be entirely commented out — `slack.Handler` returns 200 and does nothing; the live Slack integration is outbound-webhook-only. This change implements the `ack <id>` command server-side in that handler (parsing the slash-command `text` field), which brings the inbound route to life. Whether Slack actually posts to it depends on the operator's Slack app having a slash command pointed at the server — recorded as an operator-only follow-up task. The CLI ack path is fully functional regardless.
