...
The system SHALL compute, for each matched sim-vs-live pair, the per-pair deltas on the four dimensions named in the architecture doc: PnL delta (simulator PnL minus live PnL), fill-price delta (simulator fill price minus live fill price), hold-time delta (simulator hold duration minus live hold duration), and an exit-reason match flag (true when the simulator and live exit reasons are equal). All per-pair deltas SHALL be pure functions of the pair with no dependence on wall-clock time, randomness, database state, or external services.

#### Scenario: Zero-drift pair yields zero deltas and matching exit reason

- **WHEN** a pair whose simulator and live trades have identical PnL, fill price, hold time, and exit reason is compared
- **THEN** the PnL delta, fill-price delta, and hold-time delta are all zero
