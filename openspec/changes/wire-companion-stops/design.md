...
- *Durable association*: the stop's tag (`companion-stop-<entryOrderID>`). The reserved prefix is rejected for client-supplied tags at `CreateOrderRequest.Validate` — inherited by every placement ingress including the multi-leg RPC — and additionally at the PlaceOrder RPC boundary (a proper 4xx), so it cannot be poisoned from outside. The canonical tag helpers live in `models` (no import cycle); `safety` re-exports them.
...
- **Persistence**: deferrals originate from drain-once events, so an in-memory-only list loses them on halt + restart. Each deferral is persisted to the `deferred_auto_closes` table (playground ID, source order ID, serialized close request, fill parameters) when created, deleted when its close commits, and reloaded at playground load. Reload staleness is judged per request kind (re-review refinement): a CLOSE request (CloseOrderId set) is stale when its source order has no remaining open quantity — replaying a committed close would reverse the position — while an EXERCISE leg (no CloseOrderId) is independent of the option order's remaining quantity (the option close may commit while the stock leg is still owed) and is stale only when an order carrying the leg's own exercise tag already exists. A resurrected stale CLOSE row that slips past would additionally be rejected by the pending-close quantity check at placement rather than double-closing.
