...
> Adversarial review found that this change delivers the anomaly guards and their
> registry as tested, self-contained components, but does NOT wire their live
> observation feeds into the running system: `RejectionRateGuard.Observe`,
> `FillDeviationGuard.ObserveFill`, `TradesPerHourGuard.ObserveTrade`, the
> `FeedStalenessGuard` staleness signal source, and `NewGuardRegistry` have no
> production caller yet. The original requirement wording ("The system SHALL
> evaluate…") over-claimed a live end-to-end behavior that is not present. The
> requirements and scenarios below are amended in place to describe the
> **component-level** behavior actually delivered: each guard, when handed
> observations, evaluates them and engages the shared halt controller. Wiring the
> live feeds — order outcomes → the rejection sink, fills → the deviation sink,
> executed trades → the trades-per-hour sink, and the `feed-health-staleness`
> signal → the staleness guard, plus constructing the registry at startup — is
> explicitly DEFERRED to a named follow-up change, **`wire-anomaly-guard-feeds`**.
> Deferring the live wiring overnight avoids shipping unverified auto-halt
> triggers into the order path before they can be exercised end-to-end.
