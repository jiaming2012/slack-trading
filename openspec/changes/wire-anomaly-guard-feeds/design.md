...
### 4. Staleness evaluation on a ticker, gated by market hours, live-Playground presence, and a market-open grace window

A goroutine in `cmd/main.go` calls the feed-staleness evaluation on a fixed interval (default 30s). Evaluation is suppressed when the market is closed (reusing the existing market-calendar check), when no realtime Playground is loaded, and for a grace window after each market open.
...
- *Failure posture*: if the market-calendar check itself errors, evaluation is skipped for that tick with a `Warn` (fail-quiet for one interval, not fail-halt) — a calendar API blip must not halt trading; persistent calendar failure surfaces through the existing feed-health/alert paths. A calendar error also leaves the open/closed transition tracking untouched, so a blip can neither fake nor reset an open transition.
- *Market-open boundary (adversarial-review blocker fix)*: closed-market gating alone stops one hour too early — a server that sat through the overnight gap holds a last-Tick age of many hours the instant the market opens, so the first ticker cycle of the session would spuriously auto-halt most mornings before the feed's first bar arrives (up to ~95s for option feeds with no premarket). The ticker therefore tracks the closed→open transition (a server start while the market is already open counts as one) and suppresses evaluation for a **grace window equal to the staleness threshold** after each transition; after the grace elapses, evaluation proceeds normally.
  - *Alternative — require one heartbeat after the open before evaluating*: rejected; a feed that is dead FROM the open never delivers that heartbeat, so the guard would be blind on exactly the mornings it is needed most. The grace window delays the session's first evaluation without requiring an event that may never come: a dead-from-open feed still trips at open+threshold, and a feed that ticked after the open and then went quiet trips exactly per the threshold.
