...
- **BREAKING**: Replace the operator-facing crossed pairing with `Mode`: the `Meta.Environment` + `Meta.LiveAccountType` field pair is superseded by a single `Meta.Mode`, the `PlaygroundEnvironment` Go type is removed entirely, and `LiveAccountType`'s operator-facing uses are replaced by `Mode`. *(Amended 2026-07-05: `LiveAccountType` additionally serves as an internal, persisted, behavior-driving per-order/per-account tag — `order_records.account_type`, `live_accounts.account_type`, reconcile/simulator branching, mock-broker selection. Those uses are renamed to an explicitly internal `AccountRole` type with the same five string values, preserving on-disk bytes and behavior; `Mode` remains the only operator-facing selector. See design.md "Design amendment".)*
