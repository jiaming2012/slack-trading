# scanner-feature-extraction Specification

## Purpose
TBD - created by archiving change scanner-l1-l2. Update Purpose after archive.
## Requirements
...
All eight architecture-doc features (`rsi_14`, `volume_ratio`, `atr_pct`, `price_vs_50ma`, `compression_score`, `short_interest`, `sector_momentum`, `regime_tag`) SHALL be COMPUTED on the in-memory `FeatureVector` for every ticker that passes Layer 1. Of those eight, the `trading-stack-schema` `scan_results` table currently carries dedicated columns for only five: `rsi_14`, `volume_ratio`, `atr_pct`, `short_interest`, and `regime_tag`. A ticker that passes Layer 1 and completes Layer 2 SHALL result in exactly one `scan_results` row, PERSISTING those five features plus `scanner_version` and `data_as_of`. `price_vs_50ma`, `compression_score`, and `sector_momentum` are computed but NOT persisted in this change — `scan_results` has no columns for them yet. Widening the schema to add those three columns is left to a named future change, `widen-scan-results-columns`.
...
#### Scenario: A qualifying ticker produces exactly one row with the persisted subset, and the full feature set computed

- **WHEN** a `ScanInput` passes all Layer 1 gates and has sufficient history for every Layer 2 feature
- **THEN** exactly one `scan_results` row is written via `tradingstack.ScanResult`, populated with `rsi_14`, `volume_ratio`, `atr_pct`, `short_interest`, `regime_tag`, `scanner_version`, and `data_as_of`
- **AND** the in-memory `FeatureVector` built for that same input also has non-nil values for `price_vs_50ma`, `compression_score`, and `sector_momentum`, even though those three are not columns on the persisted row
