...
func FetchCandlesFromOrderRecords(symbol eventmodels.StockSymbol, orders []*eventmodels.OrderRecordDeprecated, apiKey string) ([]*eventmodels.CandleDTO, error) {
...
func ProcessBacktestTrades(symbol eventmodels.StockSymbol, orders []*eventmodels.OrderRecordDeprecated, candles []*eventmodels.CandleDTO, outDir string) (string, error) {
...
func DeriveHighestEVOrderRecord(ctx context.Context, resultCh chan map[string]interface{}, errCh chan error, event eventmodels.SignalTriggeredEvent, tradierOrderExecuter *eventmodels.TradierOrderExecuter, config *eventmodels.OptionYAML, riskProfileConstraint *eventmodels.RiskProfileConstraint, goEnv string) ([]*eventmodels.OrderRecordDeprecated, error) {
	...
	var results []*eventmodels.OrderRecordDeprecated
	for _, order := range highestEVOrderComponents {
		results = append(results, &eventmodels.OrderRecordDeprecated{
