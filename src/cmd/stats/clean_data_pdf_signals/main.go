...
func isUptrend(candle *eventmodels.CsvCandle) bool {
	if candle.UpTrend > 0 {
		return true
...
func filterCandles(candles []*eventmodels.CsvCandle, signalName string) []*eventmodels.CsvCandle {
	filteredCandles := make([]*eventmodels.CsvCandle, 0)
	filteredCandlesCount := 0

	for _, candle := range candles {
		if isUptrend(candle) && signalName == "uptrend" {
			filteredCandles = append(filteredCandles, candle)
		} else {
			filteredCandlesCount += 1
		}
	}

	log.Infof("filtered %d candles", filteredCandlesCount)
...
func main() {
	...
	csvCandles = filterCandles(csvCandles, "uptrend")
