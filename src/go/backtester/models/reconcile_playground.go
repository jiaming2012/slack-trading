	...
	if hasOppositeSides {
		if position.Quantity >= 0 {
			...
			case TradierOrderSideSell, TradierOrderSideSellShort, TradierOrderSideSellToOpen, TradierOrderSideSellToClose:
				...
				if sell_qty > 0 {
					o1 := CopyOrderRecord(r.GetId(), 0, order, AccountRoleReconcilation)
				...
				if remaining_qty := order.AbsoluteQuantity - sell_qty; remaining_qty > 0 {
					o2 := CopyOrderRecord(r.GetId(), 0, order, AccountRoleReconcilation)
		...
		o := CopyOrderRecord(r.GetId(), 0, order, AccountRoleReconcilation)
