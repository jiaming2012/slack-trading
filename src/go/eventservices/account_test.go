...
func TestUpdateConditions(t *testing.T) {
	...
	t.Run("0 entry conditions", func(t *testing.T) {
		account, err := models.NewAccount("test account", 1000, datafeed, env)
...
func TestGetStatsDownDirection(t *testing.T) {
	...
	*tf = 5
	...
	t.Run("open trades adjust after a 50% partial close", func(t *testing.T) {
		account, err := models.NewAccount("testAccount", 1000, datafeed, env)
		require.NoError(t, err)
		...
		stats, err := GetStats(id, account, &models.Tick{Price: 1.5})
		require.NoError(t, err)

		// no trades open
		require.Equal(t, 1, len(stats.Strategies))
		require.Equal(t, name, stats.Strategies[0].StrategyName)
		require.Equal(t, models.Volume(0), stats.Strategies[0].Stats.Volume)
		...
		stats, err = GetStats(id, account, &models.Tick{Price: 1.3})
		require.NoError(t, err)
		...
		require.Equal(t, models.Vwap(requestedPrice), stats.Strategies[0].Stats.Vwap)
		...
		stats, err = GetStats(id, account, &models.Tick{Price: 1.8})
		require.NoError(t, err)

		require.Equal(t, strategyVolume/2.0, stats.Strategies[0].Stats.Volume)
		require.Less(t, stats.Strategies[0].Stats.FloatingPL, models.FloatingPL(0))
		require.Less(t, stats.Strategies[0].Stats.RealizedPL, models.RealizedPL(0))
	...
	t.Run("open trades adjust after a full close via two partial closes", func(t *testing.T) {
		...
		require.InEpsilon(t, strategyVolumeV2, float64(stats.Strategies[0].Stats.Volume), models.SmallRoundingError)
		...
		require.InEpsilon(t, strategyVolumeV3, float64(stats.Strategies[0].Stats.Volume), models.SmallRoundingError)
		require.Equal(t, 1, len(stats.Strategies[0].OpenTradeLevels[tradesIndex].Trades))
...
func TestGetStatsUpDirection(t *testing.T) {
	...
	*tf = 5
	...
	t.Run("open trades adjust after a 50% partial close", func(t *testing.T) {
		...
		require.Equal(t, strategyVolume/2.0, stats.Strategies[0].Stats.Volume)
		require.Greater(t, stats.Strategies[0].Stats.FloatingPL, models.FloatingPL(0))
		require.Greater(t, stats.Strategies[0].Stats.RealizedPL, models.RealizedPL(0))
...
func TestFetchTrades(t *testing.T) {
	...
	*tf = 5
	...
	t.Run("Fetch trades is nil when no trades have been placed", func(t *testing.T) {
		account, err := models.NewAccount("test", 1000, datafeed, env)
		require.NoError(t, err)
