...
func GetCurrentStockAndOptionContracts(ctx context.Context, allOptionContracts []*models.OptionContractV1, allTrackers []*models.TrackerV3) ([]models.StockSymbol, models.OptionContracts, error) {
	allOptionContractsMap := make(map[models.OptionSymbol]*models.OptionContractV1)
	for _, contract := range allOptionContracts {
		allOptionContractsMap[contract.Symbol] = contract
	}

	allTrackersMap := make(map[models.EventStreamID]*models.TrackerV3)
	...
	stockSymbolsMap := make(map[models.StockSymbol]struct{})
	optionContractsMap := make(map[models.OptionSymbol]*models.OptionContractV1)
	...
	stockSymbols := make([]models.StockSymbol, 0, len(stockSymbolsMap))
	for s := range stockSymbolsMap {
		stockSymbols = append(stockSymbols, s)
	}

	optionContracts := make([]*models.OptionContractV1, 0, len(optionContractsMap))
