...
func (s *DatabaseService) LoadPlaygrounds(calendar *models.MarketCalendar) error {
	...
	for _, p := range playgroundsSlice {
		...
		// Rehydrate deferred option auto-closes (wire-companion-stops): a halt
		// followed by a restart must not drop closes generated by drain-once
		// assignment/expiration events.
		if err := s.restoreDeferredAutoCloses(p); err != nil {
			return fmt.Errorf("loadPlaygrounds: failed to restore deferred auto-closes for playground %s: %w", p.ID, err)
		}
	}
