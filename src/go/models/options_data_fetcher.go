...
type OptionsDataFetcher interface {
	...
	// FetchOptionChainV1 fetches option chain data based on the provided parameters. Filters options based on maxTickAge.
	FetchOptionChainV1(symbol StockSymbol, timestamp time.Time, expirationGTE, expirationLTE time.Time, maxNoOfStrikes int, minDistanceBetweenStrikes float64, expirationInDays []int, maxTickAge time.Duration, baseStrikePrice *float64, calendarRepo CalendarRepository) (*FetchOptionChainDataInput, error)
}
