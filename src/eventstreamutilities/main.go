...
func main() {
	...
	case 3:
		...
		existingOptionContracts, err := eventservices.FetchAll(ctx, esdbConn, &eventmodels.OptionContract{}, 0)
		if err != nil {
...
func StopTracking(ctx context.Context, wg *sync.WaitGroup, optionContractsCache map[eventmodels.OptionSymbol]*eventmodels.OptionContract, eventStoreDBURL string, brokerCreds BrokerCredentials) error {
	...
	// Check
	allTrackers, err := eventservices.FetchAll(ctx, esdbProducer.GetClient(), &eventmodels.Tracker{}, 0)
	if err != nil {
