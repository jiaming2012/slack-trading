...
type OptionsDataFetcher interface {
	...
	FetchOptionChainV1(symbol StockSymbol, timestamp time.Time, expirationGTE, expirationLTE time.Time, maxNoOfStrikes int, minDistanceBetweenStrikes float64, expirationInDays []int, maxTickAge time.Duration) (*FetchOptionChainDataInput, error)
}
