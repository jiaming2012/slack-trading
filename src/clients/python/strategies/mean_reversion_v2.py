"""
PDF Mean-Reversion Strategy V2 — datasource-consuming variant.

Identical to MeanReversionStrategy (V1) except _process_htf_candle() delegates
signal detection to datasources.ma_crossover.produce_signals() instead of calling
detect_atomic_signals_on_bar() inline. This is the first strategy migrated to the
TradeSignal framework datasource pattern.

Multi-timeframe equity strategy:
- HTF (1-hour, configurable): detects bullish compound signals via PDF (from datasource)
- LTF (5-min): buys dips at sigma-deviation bands when HTF is bullish
...
class MeanReversionStrategyV2(BaseStrategy):
    label = "mean_reversion_v2"
    """
    PDF-guided mean-reversion strategy (V2 — datasource signals).
