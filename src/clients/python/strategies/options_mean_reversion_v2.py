"""
Options Mean-Reversion Strategy V2 -- datasource-consuming variant.

Identical to OptionsMeanReversionStrategy (V1) except _process_htf_candle() delegates
signal detection to datasources.options_ma_crossover.produce_signals() instead of calling
detect_atomic_signals_on_bar() inline. This is the options strategy migrated to the
TradeSignal framework datasource pattern.

Multi-timeframe strategy trading options instead of shares:
- HTF (1-hour): detects bullish/bearish compound signals via PDF (from datasource)
- LTF (5-min): buys calls on dips (bullish) or puts on rallies (bearish)
- Scales in at sigma-deviation bands where P(revert) is higher
...
class OptionsMeanReversionStrategyV2(BaseStrategy):
    """
    PDF-guided options mean-reversion strategy V2 (datasource-consuming).

    Buys calls on bullish dips, puts on bearish rallies.
    Risk per group is capped at premium paid.
    """

    label = "options_mean_reversion_v2"
    ...
    def _process_htf_candle(self, bar_dict: dict) -> None:
        ...
        for sig in trade_signals:
            ...
            if direction == "bearish" and self.long_only:
                continue
            self._try_create_group(sig["signal_key"], sig["bar_dict"], sig["pdf_entry"], direction)
...
def run_options_mean_reversion_v2(
    playground: BacktesterPlaygroundClient,
    symbol: str,
    logger,
    pdf: PDFDocument,
    ...
    long_only: bool = False,
    on_tick=None,
) -> OptionsMeanReversionStrategyV2:
    """
    ...
    """
    strategy = OptionsMeanReversionStrategyV2(
