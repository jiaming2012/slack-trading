"""
Wheel Strategy V2 -- datasource-consuming variant.

Identical to WheelStrategy (V1) except check_for_put_signal() delegates
signal detection to datasources.wheel_signals.produce_open_signals() instead
of computing the feature vector inline. Extends OptionsStrategyBasicV2 (not V1
parent) per D-06.

Phase 1 (SELL_PUTS):  No stock held. Sell cash-secured OTM puts.
                      Uses wheel_signals datasource for put signal detection.
Phase 2 (SELL_CALLS): Stock held. Delegates to parent OptionsStrategyBasicV2
                      (identical to V1 pattern where WheelStrategy delegates
                      to OptionsStrategyBasic for covered call logic).
"""
...
class WheelStrategyV2(OptionsStrategyBasicV2):
    """
    Extends OptionsStrategyBasicV2 to implement the full Wheel Strategy (V2).
    ...
    V2 delegates put signal detection to wheel_signals datasource.
    Phase 2 delegates to parent OptionsStrategyBasicV2 (which delegates
    call signal detection to covered_call_signals datasource).
    """
    ...
    def check_for_put_signal(self, new_candle) -> Optional[PutSellSignal]:
        """V2: Delegate put signal detection to the wheel_signals datasource.
    ...
    def on_tick(self, tick_deltas) -> None:
        """Process tick deltas for the Wheel Strategy V2.
