...
done < ${TRADING_PROJECT_DIR}/src/.env.production-secrets

# Create the secret in the Kubernetes cluster
kubeseal --controller-name=sealed-secrets --controller-namespace=sealed-secrets --format yaml < secret.yaml > ${TRADING_PROJECT_DIR}/.clusters/production/sealedsecret.yaml