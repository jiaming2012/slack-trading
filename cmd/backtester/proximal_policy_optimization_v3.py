...
class RenkoTradingEnv(gym.Env):
    ...
    def initialize(self):
        self.client = BacktesterPlaygroundClient(self.initial_balance, 'AAPL', '2021-01-04', '2021-01-12', RepositorySource.CSV, 'training_data.csv')
    ...
    def print_current_state(self):
        ...
        if days_elapsed >= 1:
            ...
            print(f'Current time: {self._internal_timestamp}, Balance: {balance}, Commission: {self.total_commission}, PL: {pl}, Current Price: {self.current_price}, Avg Reward: {avg_reward}')

    def get_reward(self, commission):
        balance = self.client.account.balance
        result = balance - self.previous_balance - commission
    ...
    def step(self, action):
        ...
        if balance <= 0:
            ...
            terminated = True
            self._get_observation(), reward, terminated, truncated, {'balance': balance, 'pl': pl, 'position': self.position, 'current_price': self.current_price, 'ma': self.ma }
        ...
        if position > self.position >= 0:
            ...
            self.client.place_order('AAPL', self.position, OrderSide.SELL)
            cs = self.client.tick(1)
            balance = self.client.account.balance
            pl = self.client.account.pl
            ...
            self.client.place_order('AAPL', abs(self.position), OrderSide.BUY_TO_COVER)
            cs = self.client.tick(1)
            balance = self.client.account.balance
            pl = self.client.account.pl
    ...
    def reset(self, seed=None, options=None):
        ...
        print(f'Reset called (seed={seed}): balance={balance}, position={self.position}, pl={pl}, timestamp={self.timestamp}')
    ...
    def render(self, mode='human', close=False):
        balance = self.client.account.balance
        avg_reward = np.mean(self.rewards_history) if len(self.rewards_history) > 0 else 0
        print(f"Step: {self.current_step}, Tstamp: {self.timestamp}, Balance: {balance}, Commission: {self.total_commission}, SL: {self.sl}, TP: {self.tp}, Total Commission: {self.total_commission}, Avg Reward: {avg_reward}") 
