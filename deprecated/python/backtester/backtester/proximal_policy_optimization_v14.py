...
## ChatGPT prompt: get observations
def get_observations(
    ...
    # Calculate distances
    distance_to_max = max_price_est - current_price
    distance_to_min = current_price - min_price_est
    ...
    # Normalize current price within the estimated range
    if max_price_est != min_price_est:
        relative_position = (current_price - min_price_est) / (max_price_est - min_price_est)
    else:
        relative_position = 0  # Avoid division by zero if estimates are the same

    # Return the observation as a dictionary or array
    observation = {
        "current_price": current_price,
        "min_price_est": min_price_est,
        "max_price_est": max_price_est,
...
## ChatGPT prompt: reward function for closing a trade
def reward_function(
    current_price,
    min_price_est,
    max_price_est,
    ...
    if position_type == "long":
        unrealized_pnl = current_price - entry_price
        distance_to_target = max_price_est - current_price
    elif position_type == "short":
        unrealized_pnl = entry_price - current_price
        distance_to_target = current_price - min_price_est

    # Reward calculation
    reward = 0

    if action == 1:  # Close the position
        realized_pnl = unrealized_pnl - transaction_cost
        if realized_pnl > profit_threshold:
            # Normalize the closing reward based on time in position
            reward = realized_pnl + max_time_penalty / (time_in_position + 1)
        else:
            reward = -abs(realized_pnl)  # Penalize unprofitable closures

    else:  # Hold the position
        # Diminish holding rewards over time
        holding_reward = unrealized_pnl / (time_in_position + 1)

        if distance_to_target > proximity_threshold:
            # Reward holding if there's significant room for profit
            reward += holding_reward
        else:
            # Penalize holding when near the target and not closing
            reward -= (proximity_threshold / (distance_to_target + 1)) + abs(unrealized_pnl)

        # Penalize excessively long holds
        reward -= max_time_penalty / (time_in_position + 1)
...
class TradingEnv(gym.Env):
    ...
    def get_reward(self, commission, position=0, is_close=False, include_pl=False, drawdown_penalty=0.1, hold_bonus=0.1, action=0.0):
        if action > 0.5 and action < 0.6:
            return 10
        elif action < 0:
            return -10
        else:
        ...
        recent_trades = [timestamp for timestamp in self.client.trade_timestamps if timestamp >= thirty_mins_ago]
    ...
    def step(self, action):            
        ...
        reward = self.get_reward(commission, position=position, is_close=is_close, include_pl=False, action=action)
        self.rewards_history.append(reward)
    ...
    def _get_observation(self, action=0.0):
        ...
        self.current_observation = np.append([], [randomFloat]).astype(np.float64)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iterations', type=int, help='The number of iterations to train the model', default=10)
    ...
    while env.reset_count < args.iterations:    
        isDone = False
        ...
        try:
            ...
            modelName = 'ppo_model_v14-' + start_time.strftime('%Y-%m-%d-%H-%M-%S') + f'_{iterations}-terminated'
        ...
        logger.info(f'Training complete. Reset count: {env.reset_count} / {args.iterations} - iteration: {iterations}')
        ...
        if iterations % 10 == 0:
            # Save the trained model with timestep
            saveModelDir = os.path.join(projectsDir, 'slack-trading', 'cmd', 'backtester', 'models')
            modelName = 'ppo_model_v14-' + start_time.strftime('%Y-%m-%d-%H-%M-%S') + f'_{iterations}'
            modelFileName = os.path.join(saveModelDir, modelName)
            model.save(modelFileName)
            save_meta(modelFileName, args, params, iterations)
    ...
    saveModelDir = os.path.join(projectsDir, 'slack-trading', 'cmd', 'backtester', 'models')
    modelName = 'ppo_model_v14-' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    modelFileName = os.path.join(saveModelDir, modelName)
    model.save(modelFileName)
    save_meta(modelFileName, args, params, iterations)
