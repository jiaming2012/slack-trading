...
# === ADD EGARCH model ===
print("\nFitting EGARCH(1,1)...")
model = arch_model(log_returns, vol='EGARCH', p=1, o=1, q=1, dist='normal')
...
print(f"\nSimulating {n_paths} future paths of {horizon} days...")
# sim = res.simulate(res.params, nobs=horizon, repetitions=n_paths)
sim = res.model.simulate(res.params, nobs=horizon)

# sim_returns = sim['data']  # shape: (horizon, n_paths)

# === CONVERT TO FUTURE PRICES ===
S0 = prices.iloc[-1]
...
for _ in range(n_paths):
    sim = res.model.simulate(res.params, nobs=horizon)
    path_returns = sim['data'].values
    paths.append(path_returns)

sim_returns = np.array(paths).T  # shape (horizon, n_paths)
sim_prices = S0 * np.exp(np.cumsum(sim_returns, axis=0))


# === Plot terminal prices (optional) ===
plt.hist(sim_prices[-1], bins=50, edgecolor='k')
plt.axvline(S0, color='g', linestyle='--', label='Start Price')
plt.title(f"Simulated Terminal Prices for {symbol}")
plt.legend()
plt.show()

# === Optional: Estimate EV of short put ===
strike = S0 * 0.97   # Example: 3% OTM put
premium = 1.50       # Example premium

terminal_prices = sim_prices[-1]
profits = np.where(
    terminal_prices >= strike,
    premium,
    terminal_prices - strike + premium
)

expected_value = profits.mean()
print(f"\nExpected Value of Selling ${strike:.2f} Put for ${premium:.2f}: ${expected_value:.2f}")